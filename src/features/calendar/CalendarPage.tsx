...
export function useCalendarEvents(){
  ...
  const events=useMemo<CalendarEvent[]>(()=>{
    ...
    for(const card of cards){if(card.status!=='aktif')continue;const dueDate=resolveCardDueDate(card,statements).date;result.push({id:`card-${card.id}`,date:dateKey(dueDate),title:`${card.bank} son ödeme`,detail:`${card.cardName} •••• ${card.last4}`.trim(),type:'credit-card',to:`/finance/credit-cards/${card.id}`})}
