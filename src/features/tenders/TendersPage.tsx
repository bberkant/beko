...
const emptyForm=():FormState=>({tenderNumber:'',title:'',institution:'',tenderType:'hizmet',method:'acik',status:'hazirlaniyor',estimatedAmount:'',bidAmount:'',currency:'TRY',publicationDate:'',deadlineAt:'',resultDate:'',assignedTo:'',description:'',teminatMektubu:''});
...
export function TendersPage(){
  ...
  const [activeErpTab, setActiveErpTab] = useState<'ihaleler' | 'satinalma' | 'talep'>('ihaleler');
  ...
  const save=async()=>{
    if(!user?.organizationId||!form.tenderNumber.trim()||!form.title.trim()||!form.institution.trim()||!form.deadlineAt){
      notify('İhale no, başlık, kurum ve son teklif tarihi zorunludur.','error');
      return;
    }
    setSaving(true);
    const payload={
    ...
    notify(editing?'İhale güncellendi.':'İhale eklendi.','success');
    setOpen(false);
    await refresh()
  };
  
  const remove=async(item:Tender)=>{
    if(!confirm(`“${item.title}” ihalesi silinsin mi?`))return;
    const{error}=await supabase.from('tenders').delete().eq('id',item.id).eq('organization_id',user?.organizationId);
    if(error)notify(error.message,'error');
    else{notify('İhale silindi.','success');await refresh()}
  };
  ...
  const removeAll = async () => {
    ...
    if (targetItems.length === 0) {
      notify('Silinecek ihale bulunamadı.', 'info');
      return;
    }
    const count = targetItems.length;
    const msg = count === items.length
      ? `Tüm ihaleleri (${count} adet) silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`
      : `Filtrelenen ${count} adet ihaleyi silmek istediğinize emin misiniz? Bu işlem geri alınamaz.`;
    ...
    if (!hasError) {
      notify(`${count} adet ihale başarıyla silindi.`, 'success');
      await refresh();
    }
  };
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <PageHeader 
        title="İhaleler" 
        description="İhale süreçlerini, teklif tarihlerini, teminat mektuplarını ve sonuçları yönetin." 
        actions={
          <>
            <EkapCandidates canWrite={canWrite} onAccepted={refresh}/>
      ...
      {sidebarTheme === 'bulut_erp' && (
        <div className="flex border-b border-gray-200 mb-6">
          <button
            onClick={() => setActiveErpTab('ihaleler')}
            className={`px-4 py-2 text-sm font-semibold border-b-2 transition-all ${
              activeErpTab === 'ihaleler' ? 'border-[#f37021] text-[#f37021]' : 'border-transparent text-gray-500 hover:text-gray-700'
            }`}
          >
            İhale Yönetimi (Ekap)
          </button>
      ...
      {activeErpTab === 'ihaleler' && (
        <>
          <div className="mb-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Metric icon={<Gavel size={17}/>} value={String(items.length)} label="Toplam İhale"/>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row">
        <div className="relative max-w-lg flex-1">
          ...
          <input className="input pl-9" value={query} onChange={e=>setQuery(e.target.value)} placeholder="İhale no, başlık, kurum veya teminat mektubu ara..."/>
        </div>
      ...
      <div className="card overflow-x-auto">
        <table className="min-w-full">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} className="table-td py-12 text-center text-gray-400">İhaleler yükleniyor...</td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={7} className="table-td py-12 text-center text-gray-400">İhale bulunamadı.</td></tr>
            ) : (
              filtered.map(x => (
                <tr key={x.id} className="border-t border-gray-100">
                  <td className="table-td max-w-[280px]">
                    <a 
                      ...
                      title={`${x.title} (EKAP'ta ara - İKN panoya kopyalanır)`}
                    >
                      {x.title}
                    </a>
      ...
      {activeErpTab === 'satinalma' && (
        <div className="card p-6 bg-white border border-gray-200 rounded-2xl shadow-sm">
          ...
          <p className="text-xs text-gray-500 mb-6">Tedarikçilere geçilen siparişlerin onay, sevkıyat ve faturalandırılma süreçlerini anlık izleyin.</p>
      ...
      {activeErpTab === 'talep' && (
        <div className="card p-6 bg-white border border-gray-200 rounded-2xl shadow-sm">
          ...
          <p className="text-xs text-gray-500 mb-6">Şirket içi departmanların satın alma taleplerinin bütçe limitlerine göre incelenmesi ve onaylanması.</p>
      ...
      <Modal open={open} onClose={()=>setOpen(false)} title={editing?'İhaleyi Düzenle':'Yeni İhale'} size="lg">
        <div className="grid gap-4 sm:grid-cols-2">
          <Field label="İhale No"><input className="input" value={form.tenderNumber} onChange={e=>setForm({...form,tenderNumber:e.target.value})}/></Field>
          ...
          <Field label="İhale Türü">
            <select className="input" value={form.tenderType} onChange={e=>setForm({...form,tenderType:e.target.value})}>
          ...
          <Field label="Yöntem">
            <select className="input" value={form.method} onChange={e=>setForm({...form,method:e.target.value})}>
              ...
              <option value="dogrudan">Doğrudan Temin</option>
