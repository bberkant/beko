...
const cls: any = {
  ...
  satildi: "bg-red-50 text-red-700",
};
const soon = (d?: string) =>
...
export function VehicleListPage() {
  ...
  const { vehicles, expenses, deleteVehicle } = useVehicles();
  ...
  return (
    <div className="mx-auto max-w-7xl">
      <PageHeader
        ...
        actions={<><ModuleFileActions module="vehicles" exportName="arac-listesi" rows={vehicles.map(v=>({Plaka:v.plate,Marka:v.brand,Model:v.model,"Model Yılı":v.modelYear,Kilometre:v.currentKm,"Kullanan":v.assignedTo,Departman:v.department,Muayene:v.inspectionDate,Sigorta:v.insuranceDate,Durum:v.status}))}/><button className="btn-primary" onClick={() => nav("/arac-yonetimi/yeni")}><Plus size={16} />Yeni Araç</button></>}
      />
      <div className="mb-6 grid gap-4 sm:grid-cols-4">
        ...
        <K
          ...
          v={String(
            vehicles.filter(
              (v) => soon(v.inspectionDate) || soon(v.insuranceDate),
            ).length,
          )}
          l="Yaklaşan Belge"
        />
        ...
        <table className="min-w-full">
          <thead>
            <tr>
              ...
              <th className="table-th">Kullanan</th>
              <th className="table-th">Kilometre</th>
              <th className="table-th">Muayene</th>
              <th className="table-th">Sigorta</th>
          ...
          <tbody>
            {list.map((v) => (
              <tr key={v.id} className="border-t">
                ...
                <td className="table-td">{v.assignedTo || "—"}</td>
                <td className="table-td">
                  {v.currentKm.toLocaleString("tr-TR")} km
                </td>
                <td
                  className={`table-td ${soon(v.inspectionDate) ? "text-red-600 font-medium" : ""}`}
                >
                  {v.inspectionDate || "—"}
                </td>
                <td
                  className={`table-td ${soon(v.insuranceDate) ? "text-red-600 font-medium" : ""}`}
                >
                  {v.insuranceDate || "—"}
                </td>
                <td className="table-td">
                  <Badge className={cls[v.status]}>{v.status}</Badge>
                </td>
                <td className="table-td">
                  <button
