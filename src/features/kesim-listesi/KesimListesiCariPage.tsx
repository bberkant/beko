...
export function KesimListesiCariPage() {
  ...
  const renderMainSortHeader = (label: string, field: string, align: 'left' | 'right' | 'center' = 'left', extraClass = '') => {
    const isSorted = sortField === field;
    return (
      <th 
        onClick={() => handleSort(field)}
        className={`px-3 py-3 cursor-pointer select-none hover:bg-gray-100/50 transition-colors font-semibold uppercase tracking-wider text-[11.5px] text-gray-500 ${align === 'right' ? 'text-right pr-8' : align === 'center' ? 'text-center' : 'text-left'} ${extraClass}`}
      >
  ...
  const handleOpenAdd = () => {
    ...
    if (selectedSupplier) {
      initialForm.supplier = selectedSupplier;
    }
  ...
  const filteredItems = useMemo(() => {
    return records.filter((item) => {
      // If a specific supplier is selected, only show their records
      if (selectedSupplier && item.supplier.trim() !== selectedSupplier.trim()) return false;

      // General Search (Supplier) - only apply in the main view
      if (!selectedSupplier && search) {
  ...
  }, [records, search, animalTypeFilter, startDate, endDate, columnFilters, selectedSupplier]);

  const aggregatedCaris = useMemo(() => {
    const groups: Record<string, AggregatedCari> = {};

    const itemsToGroup = selectedSupplier 
      ? records.filter(item => {
          if (search) {
            const query = search.toLocaleLowerCase('tr-TR');
            const matchesSupplier = item.supplier.toLocaleLowerCase('tr-TR').includes(query);
    ...
    itemsToGroup.forEach(item => {
      ...
      if (!groups[key]) {
        groups[key] = {
          ...
          pesinat: 0,
          kalan_tutar: 0,
          last_slaughter_date: item.slaughter_date
        };
      }
      groups[key].head_count += item.head_count || 0;
      groups[key].carcass_weight += item.carcass_weight || 0;
      groups[key].total_amount += item.total_amount || 0;
      groups[key].pesinat += item.pesinat || 0;
      groups[key].kalan_tutar += item.kalan_tutar || 0;
      if (item.slaughter_date > groups[key].last_slaughter_date) {
        groups[key].last_slaughter_date = item.slaughter_date;
      }
    });

    const result = Object.values(groups);
    result.sort((a, b) => {
      let aVal: any = a[sortField as keyof AggregatedCari];
      let bVal: any = b[sortField as keyof AggregatedCari];

      if (sortField === 'avg_price') {
        aVal = a.carcass_weight > 0 ? (a.total_amount / a.carcass_weight) : 0;
        bVal = b.carcass_weight > 0 ? (b.total_amount / b.carcass_weight) : 0;
      }

      if (typeof aVal === 'string') {
        const comp = aVal.localeCompare(bVal, 'tr-TR');
    ...
    return result;
  }, [records, filteredItems, selectedSupplier, search, animalTypeFilter, startDate, endDate, sortField, sortDirection]);
        ...
        <div className="rounded-xl border border-gray-150 bg-white p-5 shadow-sm transition-all hover:shadow-md">
          <div className="flex items-center justify-between">
            <span className="text-[12.5px] font-bold text-gray-400 uppercase tracking-wider">
              {selectedSupplier ? "Toplam Adet" : "Toplam Cari"}
            </span>
            <div className="rounded-lg bg-brand-50 p-2 text-brand-600">
              {selectedSupplier ? <Beef size={20} /> : <UserCheck size={20} />}
            </div>
          </div>
          <div className="mt-4 flex items-baseline gap-2">
            <span className="text-2xl font-bold tracking-tight text-gray-900">
              {selectedSupplier ? stats.totalCount : aggregatedCaris.length}
            </span>
            <span className="text-sm font-semibold text-gray-500">
              {selectedSupplier ? "baş" : "kişi"}
            </span>
          </div>
        </div>
          ...
          {selectedSupplier ? (
            ...
            <thead className="text-[11.5px] uppercase tracking-wider text-gray-500">
              <tr className="bg-gray-50 sticky top-16 z-20 shadow-[0_1px_0_0_rgba(229,231,235,0.1)]">
                ...
                <th className="px-3 py-3 font-semibold text-left min-w-[180px]">Tedarikçi / Cari Hesap</th>
              ...
              {loading ? (
                ...
                filteredItems.map((item) => (
                  <tr key={item.id} className="hover:bg-gray-50/50 transition-colors text-sm">
                    ...
                    <td className="px-3 py-1.5 text-left font-bold text-blue-600 text-[15px]">
                      <InlineEdit
                        value={item.supplier}
                        displayValue={item.supplier}
                        onSave={(val) => void handleSaveField(item.id, 'supplier', val)}
                        className="font-bold text-blue-600 hover:text-blue-800 text-[15px]"
                      />
                    </td>
                    ...
                    <td className="px-3 py-1.5 text-right pr-8 font-bold text-gray-950 text-[15px]">
                      <InlineEdit
                        ...
                        isNumeric={true}
                        align="right"
                        className="justify-end font-bold text-gray-950 text-[15px]"
                        inputClassName="!text-right font-bold text-gray-950 text-[15px]"
                      />
                    </td>
            ...
            {filteredItems.length > 0 && (
              <tfoot className="bg-gray-50 border-t-2 border-gray-200 font-bold text-gray-900">
                <tr>
                  ...
                  <td className="px-3 py-3 text-right text-brand-700 text-sm">{formatCurrency(stats.totalRemaining)}</td>
            ...
            <table className="w-full border-collapse text-left text-sm">
              <thead className="text-[11.5px] uppercase tracking-wider text-gray-500">
                <tr className="bg-gray-50 sticky top-16 z-20 shadow-[0_1px_0_0_rgba(229,231,235,0.1)]">
                  {renderMainSortHeader('Tedarikçi / Cari Hesap', 'supplier', 'left', 'min-w-[200px]')}
                  {renderMainSortHeader('Toplam Adet', 'head_count', 'center', 'w-[120px]')}
                  {renderMainSortHeader('Toplam Ağırlık', 'carcass_weight', 'right', 'w-[150px]')}
                  {renderMainSortHeader('Ortalama Fiyat', 'avg_price', 'right', 'w-[150px]')}
                  {renderMainSortHeader('Toplam Tutar', 'total_amount', 'right', 'w-[180px]')}
                  {renderMainSortHeader('Toplam Ödenen', 'pesinat', 'right', 'w-[150px]')}
                  {renderMainSortHeader('Kalan Borç / Bakiye', 'kalan_tutar', 'right', 'w-[180px]')}
                  {renderMainSortHeader('Son İşlem Tarihi', 'last_slaughter_date', 'center', 'w-[150px]')}
                  <th className="px-3 py-3 font-semibold text-center w-[120px] text-gray-500 uppercase tracking-wider text-[11.5px]">İşlemler</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-100 bg-white">
                {loading ? (
                  ...
                  aggregatedCaris.map((item) => {
                    const avgPrice = item.carcass_weight > 0 ? (item.total_amount / item.carcass_weight) : 0;
                    return (
                      <tr key={item.supplier} className="hover:bg-gray-50/50 transition-colors text-sm font-medium text-gray-700">
                        ...
                        <td className="px-3 py-2.5 text-right pr-8 font-semibold text-gray-800">
                          {formatNumber(item.carcass_weight, 'kg')}
                        </td>
                        <td className="px-3 py-2.5 text-right pr-8 text-gray-600 font-mono">
                          {formatCurrency(avgPrice)}
                        </td>
                        <td className="px-3 py-2.5 text-right pr-8 text-gray-950 font-bold text-[15px]">
                          {formatCurrency(item.total_amount)}
                        </td>
                        <td className="px-3 py-2.5 text-right pr-8 text-amber-700 font-semibold">
                          {item.pesinat > 0 ? formatCurrency(item.pesinat) : '—'}
                        </td>
                        <td className="px-3 py-2.5 text-right pr-8 text-brand-700 font-bold text-[14px]">
                          {formatCurrency(item.kalan_tutar)}
                        </td>
                        <td className="px-3 py-2.5 text-center text-gray-500 text-xs">
                          {formatDate(item.last_slaughter_date)}
                        </td>
              ...
              {aggregatedCaris.length > 0 && (
                <tfoot className="bg-gray-50 border-t-2 border-gray-200 font-bold text-gray-900">
                  <tr>
                    <td className="px-3 py-3 text-right text-gray-500 uppercase tracking-wider text-[11px] font-bold">Toplamlar:</td>
                    <td className="px-3 py-3 text-center text-gray-900">{stats.totalCount} baş</td>
                    <td className="px-3 py-3 text-right text-gray-800 pr-8">{formatNumber(stats.totalCarcassWeight, 'kg')}</td>
                    <td className="px-3 py-3 text-right"></td>
                    <td className="px-3 py-3 text-right text-gray-950 pr-8">{formatCurrency(stats.totalAmount)}</td>
                    <td className="px-3 py-3 text-right text-amber-700 pr-8">
                      {aggregatedCaris.reduce((sum, item) => sum + item.pesinat, 0) > 0 
                        ? formatCurrency(aggregatedCaris.reduce((sum, item) => sum + item.pesinat, 0)) 
                        : '—'}
                    </td>
                    <td className="px-3 py-3 text-right text-brand-700 text-sm pr-8">{formatCurrency(stats.totalRemaining)}</td>
                    <td colSpan={2} className="px-3 py-3"></td>
                  </tr>
                </tfoot>
              )}
